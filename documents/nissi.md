## ページを修正したり削除したりした場合 2020/05/26

ページ削除するとidがずれる

- noindexとか404を返すとかいうのをjsonに書いて指示するのはちょっと面倒

- ページ削除の代わりに空のjsonを置いておく、というのが簡単。でもその場合「ページ削除されてる」と認識されるのかは不明（要検証、される気もする）
↑ダメ。記事一覧には表示されちゃうじゃん。

もしidをパーマリンクにするなら、jsonに手動で指定したidを振っていく方が良いのではないか。スマートではないけど決してずれないし、idに数字以外のものを振る柔軟な運用もできるかも。
その代わり、id = data.length - i みたいな単純計算はできなくなるので要修正。



やっぱりjsで生成したページのクローラー巡回が遅い気がする。多分ふだんはSEOなんて気にしないから、id=9だけが懸案事項なんだけど。
